...
function presetForSkill(skillName) {
  const n = normalizeSkillName(skillName).toLowerCase();

  const presets = [
    {
      match: /(openapi|swagger|api design|api design)/i,
      learn: [
...
function tasksFor(skillName, status, requiredLevel, userLevel, roleTitle, weekNo) {
  const s = normalizeSkillName(skillName) || "this skill";
  const req = safeNum(requiredLevel, 1);
  const have = safeNum(userLevel, 0);
